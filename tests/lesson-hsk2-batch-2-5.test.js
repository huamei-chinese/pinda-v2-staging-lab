...
test('HSK2 lessons 2-5 content pack exists and each lesson follows the expected shape', async () => {
  const batch = require('../lesson-hsk2-batch-2-5.js');

  assert.equal(batch.level, 'HSK2');
  assert.equal(Array.isArray(batch.lessons), true);
  assert.equal(batch.lessons.length, 4);

  const ids = batch.lessons.map((lesson) => lesson.id);
  assert.deepEqual(ids, ['hsk2-l2', 'hsk2-l3', 'hsk2-l4', 'hsk2-l5']);
  ...
  for (const lesson of batch.lessons) {
    ...
    assert.ok(!lesson.titleZh.includes('�'));
    assert.ok(!lesson.titleVi.includes('�'));
