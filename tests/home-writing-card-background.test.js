...
test("home writing duo card uses the supplied writing background with readable overlay", () => {
  assert.match(
    styles,
    /\.screen-home \.home-duo-card--write,[\s\S]*\.screen-home \.home-desktop-module-card\.home-duo-card--write\s*\{[\s\S]*url\("assets\/backgroundluyenviet\.png"\) center \/ cover no-repeat !important;/,
  );
