...
test("listening dashboard hero title follows selected dialogue level", () => {
  ...
  assert.match(appSource, /"dialogue-so-cap": isVi \? "Nghe Sơ Cấp"/);
  assert.match(appSource, /"dialogue-trung-cap": isVi \? "Nghe Trung Cấp"/);
  assert.match(appSource, /"dialogue-cao-cap": isVi \? "Nghe Cao Cấp"/);
