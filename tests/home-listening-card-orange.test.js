...
test("home listening duo card uses the supplied listening artwork with a blue readability wash", () => {
  assert.match(
    styles,
    /\.screen-home \.home-duo-card--listen,[\s\S]*\.screen-home \.home-desktop-module-card\.home-duo-card--listen\s*\{[\s\S]*rgba\(7,\s*89,\s*133,\s*0\.5\)[\s\S]*rgba\(14,\s*165,\s*233,\s*0\.5\)[\s\S]*url\("assets\/backgroundluyennghe\.png"\) center \/ cover no-repeat !important;/i,
  );
