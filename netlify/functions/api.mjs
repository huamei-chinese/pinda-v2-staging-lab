...
function speechAssessmentProvider() {
  ...
  const hasSpeechace = Boolean(env("SPEECHACE_API_KEY"));
...
function speechaceApiKey() {
  const key = env("SPEECHACE_API_KEY");
  try {
...
async function assessWithSpeechace(referenceText, audioBuffer, mimeType) {
  ...
  const endpoint = env("SPEECHACE_ENDPOINT") || "https://api.speechace.co/api/scoring/text/v9/json";
  const dialect = env("SPEECHACE_DIALECT") || "en-us";
  const userId = env("SPEECHACE_USER_ID") || "huamei-listening";
  const url = new URL(endpoint);
  url.searchParams.set("key", key);
  url.searchParams.set("dialect", dialect);
  url.searchParams.set("user_id", userId);
  url.searchParams.set("include_fluency", env("SPEECHACE_INCLUDE_FLUENCY") || "1");
  ...
  if (!response.ok || data?.status === "error") {
    throw apiError(data?.message || data?.detail || data?.error || "Khong cham duoc phat am bang Speechace.", response.status || 502);
  }
