...
- Tests: `npm.cmd test` passes 137/137
- Build: `npm.cmd run build` passes
- One-command acceptance gate: `npm.cmd run acceptance:lab` runs safety, tests, build, audit, starts `server.js`, runs 17 smoke checks, and shuts the local server down.
- Browser QA: desktop and mobile lab surfaces were checked with the in-app browser; HSK cards fit at 1280 x 720, and HSK2 lesson 1 opens local fallback study parts on mobile without a VIP modal when `DATABASE_URL` is missing.
...
- `npm.cmd test` passes 137/137 or the updated full suite passes with a documented new count.
